...
class ShortFollowupQuestion(BaseModel):
    """Structured output for short conversational follow-up questions"""
    short_followup: str = Field(description="Short conversational follow-up question with implicit reference")
    short_answer: str = Field(description="Answer to the short follow-up question")
...
class HybridConfig:
    ...
    questions_per_document: int = 50  # Total questions per document
    ...
    # Generation Split
    ragas_percentage: float = 0.50  # 50% RAGAS
    custom_percentage: float = 0.50  # 50% Custom
    ...
    # Multi-turn - for existing chatbot style questions
    custom_multiturn_ratio: float = 0.50
    max_turns_per_conversation: int = 2
    
    # Short conversational follow-ups - NEW
    short_conversational_ratio: float = 0.30  # 30% of chatbot questions get short follow-ups
...
def main():
    ...
    print(f"   RAGAS: {ragas_count} ({int(config.ragas_percentage*100)}%)")
    print(f"   Custom: {custom_count} ({int(config.custom_percentage*100)}%)")
    ...
    short_followup_llm = base_llm.with_structured_output(ShortFollowupQuestion)
    ...
    print(f"\n📍 RAGAS Generation ({int(config.ragas_percentage*100)}%)...")
    
    ragas_df = generate_ragas_questions(
        base_llm, embeddings_model, sampled_for_ragas, config
    )
    
    print(f"✅ RAGAS generated {len(ragas_df)} questions")
    
    # =====================================================================
    # CUSTOM GENERATION (50%) WITH STRUCTURED OUTPUT
    # =====================================================================
    
    print(f"\n📍 Custom Generation ({int(config.custom_percentage*100)}%) with structured output...")
...
def sample_chunks_spatial(chunks: List[Document], n_samples: int, num_regions: int) -> List[Document]:
    ...
    samples_per_region = max(1, n_samples // num_regions)
...
def generate_ragas_questions(llm, embeddings, sampled_chunks, config):
...
def generate_custom_questions_structured(sampled_chunks, question_llm, followup_llm, short_followup_llm, config):
    ...
    for source_file, chunks in sampled_chunks.items():
        ...
        # Determine multi-turn (existing long follow-ups)
        ...
        # Determine short conversational follow-ups (NEW)
        chatbot_indices = [i for i, qt in enumerate(q_types) if qt == 'chatbot_style']
        n_short_followups = int(len(chatbot_indices) * config.short_conversational_ratio)
        short_followup_indices = []
        if n_short_followups > 0 and len(chatbot_indices) > 0:
            short_followup_indices = np.random.choice(
                chatbot_indices, 
                min(n_short_followups, len(chatbot_indices)), 
                replace=False
            ).tolist()
        ...
        for idx, (chunk, q_type) in enumerate(tqdm(list(zip(chunks, q_types)), desc="Custom Gen")):
            ...
            if qa:
                ...
                qa['has_short_followup'] = False
                qa['short_followup_questions'] = []
                
                # Generate existing long follow-up if multi-turn
                ...
                # Generate NEW short conversational follow-up for chatbot questions
                if idx in short_followup_indices and q_type == 'chatbot_style':
                    short_followup = generate_short_followup_with_structure(qa, chunk, short_followup_llm)
                    if short_followup:
                        qa['has_short_followup'] = True
                        qa['short_followup_questions'] = [short_followup]
                        # If has short followup, update conversation type
                        if not qa['has_followup']:
                            qa['conversation_type'] = 'multi_turn'
                
                doc_questions.append(qa)
        
        all_questions.extend(doc_questions)
        print(f"✅ Generated {len(doc_questions)} questions")
        print(f"   Long follow-ups: {sum(1 for q in doc_questions if q['has_followup'])}")
        print(f"   Short conversational follow-ups: {sum(1 for q in doc_questions if q['has_short_followup'])}")
...
TYPE DEFINITIONS:
- chatbot_style: Conversational ("Can you help me understand...?", "Who is...?", "What does X mean?")
...
- For chatbot_style: Use natural, conversational language
...
def generate_followup_with_structure(initial_qa: Dict, chunk: Document, llm) -> Optional[Dict]:
    """Generate follow-up question using structured output (existing logic)"""
...
def generate_short_followup_with_structure(initial_qa: Dict, chunk: Document, llm) -> Optional[Dict]:
    """Generate SHORT conversational follow-up question (NEW)"""
    prompt = f"""Generate a SHORT conversational follow-up question.

CONTEXT: {chunk.page_content[:2500]}
INITIAL QUESTION: {initial_qa['question']}
INITIAL ANSWER: {initial_qa['answer']}

REQUIREMENTS:
1. Make it SHORT (5-10 words maximum)
2. Make it CONVERSATIONAL and NATURAL
3. Use IMPLICIT reference (pronouns like "his", "it", "that", "they") - DO NOT repeat the subject name
4. Build on the previous answer
5. Ask for additional details, clarification, or related information

EXAMPLES OF GOOD SHORT FOLLOW-UPS:
...
Generate ONE short conversational follow-up question with implicit reference.
"""
    
    try:
        result: ShortFollowupQuestion = llm.invoke(prompt)
        return {
            'followup_question': result.short_followup,
            'followup_answer': result.short_answer,
            'followup_type': 'short_conversational'
        }
    except Exception as e:
        return None
...
def merge_datasets(ragas_df: pd.DataFrame, custom_questions: List[Dict]) -> pd.DataFrame:
    ...
    for qa in custom_questions:
        ...
        if qa.get('has_followup'):
            ...
            for followup in qa.get('followup_questions', []):
                fu = {
                    ...
                    'source_file': qa.get('source_file'),
                    'region_id': qa.get('region_id', 0) + 1,
                    'question_type': 'followup_long',
                    'question_style': 'followup_long',
        ...
        # Short conversational follow-ups (NEW)
        if qa.get('has_short_followup'):
            parent_idx = len(custom_flat) - 1
            for short_followup in qa.get('short_followup_questions', []):
                sfu = {
                    'question': short_followup['followup_question'],
                    'answer': short_followup['followup_answer'],
                    'source_file': qa.get('source_file'),
                    'region_id': qa.get('region_id', 0) + 1,
                    'question_type': 'followup_short_conversational',
                    'question_style': 'followup_short_conversational',
...
def print_coverage_analysis(df: pd.DataFrame):
    ...
    # Show short conversational follow-ups
    short_followups = len(df[df['question_type'] == 'followup_short_conversational'])
    if short_followups > 0:
        print(f"\nShort conversational follow-ups: {short_followups}")
...
def print_sample_questions(df: pd.DataFrame):
    ...
    # Show short conversational examples
    short_samples = df[df['question_type'] == 'followup_short_conversational'].head(2)
    if len(short_samples) > 0:
        print("\n🔹 SHORT CONVERSATIONAL FOLLOW-UPS:\n")
        for _, row in short_samples.iterrows():
            parent_id = row['parent_question_id']
            if parent_id is not None and parent_id < len(df):
                parent = df.iloc[parent_id]
                print(f"Parent Q: {parent['question']}")
                print(f"Parent A: {parent['answer'][:100]}...")
                print(f"Follow-up: {row['question']}")
                print(f"Follow-up A: {row['answer'][:100]}...\n")
...
def export_datasets(df: pd.DataFrame):
    ...
    summary = {
        ...
        'short_conversational_followups': len(df[df['question_type'] == 'followup_short_conversational']),
...
def create_visualizations(df: pd.DataFrame):
    ...
    axes[0, 1].set_yticklabels(type_counts.index, fontsize=8)
